...
- Automatic trimming of surrounding spaces in discovered/imported book paths
...
BOOKS_ROOT=/opt/breaderos/static/books
```
...
It does not bind-mount the whole repository into runtime containers.
...
- Docker/container run uses `BOOKS_ROOT=/opt/breaderos/static/books`
...
- On access, the app normalizes leading/trailing spaces in book file and directory names under `static/books`
- Compose can override the host books mount through `BOOKS_HOST_PATH`, while runtime containers stay on dedicated minimal mounts
...
Imported trees are normalized automatically after copy. If normalization would create sibling-name collisions, the import is rolled back.
